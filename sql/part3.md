...
Note that `\` commands like `\copy` in `psql` need to be all on one line withouth comments in them.
...
We're making the player id a serial type to automatically assign ID numbers.  This doesn't prevent us from supplying our own ids, but doing so will potentially conflict with the autogenerated sequence ([more info](https://stackoverflow.com/questions/244243/how-to-reset-postgres-primary-key-sequence-when-it-falls-out-of-sync)).  Since we have to link players to teams later, we'll need to know what both the player ID and team ID are before we do the linking.  So we might want to make this column just an int instead and assign IDs ourselves.  The downside of that is keeping track of what ids have already been used.
