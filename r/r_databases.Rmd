...
The `DBI` package has the core functionality of connecting R to database servers.  There are then packages that implement the core functionality of `DBI` for each specific implementation of SQL.  A package for PostgreSQL is `RPostgres`. (Note: this workshop previously used `RPostgreSQL` but the newer package `RPostgres` is more fully compliant with the `DBI` API.)
